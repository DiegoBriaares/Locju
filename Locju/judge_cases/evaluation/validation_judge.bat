...
if not exist %cd%\validator\validator.cpp set /p ex=Press Enter
...
if not exist %cd%\validator\validator.exe echo [91mCompilation Failed[0m & del sol.exe & exit
...
if not exist %pat%\%test_in% echo %result% & echo ------------------------------------------------------------------------------ & set /p ex=Successfully judged & del sol.exe & exit
