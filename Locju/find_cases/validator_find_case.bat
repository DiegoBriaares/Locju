...
if not exist sol.cpp set /p ex=Press Enter
...
if not exist sol.exe echo [91mCompilation Failed[0m & set /p ex=press any key & exit
cd generator
if not exist generator.cpp echo [91mRequired "generator.cpp"[0m 
if not exist generator.cpp set /p ex=Press Enter
...
if not exist generator.exe echo [91mCompilation Failed[0m & set /p ex=press any key & exit
cd ..
cd correct_solution
if not exist correct_sol.cpp echo [91mRequired "correct_sol.cpp"[0m 
if not exist correct_sol.cpp set /p ex=Press Enter
...
if not exist correct_sol.exe echo [91mCompilation Failed[0m & set /p ex=press any key & exit
cd ..
cd validator
if not exist validator.cpp echo [91mRequired "validator.cpp"[0m 
if not exist validator.cpp set /p ex=Press Enter
if not exist validator.cpp exit
set /a comp=0
if not exist validator.exe echo [95mCompiling "validator.cpp"..[0m & g++ validator.cpp -o validator.exe -ggdb -DDEBUG -std=c++17 -O2 -Wl,--stack=256000000 -Wall -Wextra -Wshadow -DLOCAL & set /a comp=1
if not exist validator.exe echo [91mCompilation Failed[0m & set /p ex=press any key & exit
cd ..
set /p rep=[1mNumber of testcases to generate: [0m
if %rep% LSS 0 echo ------------------------------------------------------------------------------ & echo [91mA positive number please[0m & set /p ex=Press any key to try again & exit
...
validator_search_case.bat %rep% %rep% %project%
